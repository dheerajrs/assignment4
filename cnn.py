...
class CNN(object):
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        layer_obj = keras.layers.Conv2D(filters=num_of_filters, kernel_size=kernel_size, padding=padding, strides=strides, activation=activation, name=name, trainable=trainable)
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        layer_obj = keras.layers.MaxPooling2D(pool_size=pool_size, padding=padding, strides=strides, name=name)
        self.model.add(layer_obj)
        return layer_obj
    ...
    def append_flatten_layer(self,name=""):
        """
         ...
         :return: Layer object
         """
        layer_obj = keras.layers.Flatten(name=name)
        self.model.add(layer_obj)
        return layer_obj
